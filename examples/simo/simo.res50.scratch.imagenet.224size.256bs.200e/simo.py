...
class NT_Xent(nn.Module):
    ...
    def forward(self, z_i, z_j):
        ...
        for idx in range(comm.get_world_size()):
            if idx == local_rank:
                # current device
                print(z_j.shape, z_j.requires_grad)
                z_large.append(z_j)
            else:
        ...
        sim_i_large = self.similarity_f(
            z_i.unsqueeze(1), z_large.unsqueeze(0)) / self.temperature
        ...
        negative_samples_i = sim_i_large[neg_mask_i].reshape(device_size, -1)[:, neg_perm]
        ...
        loss_i = torch.log(
            ...
            # self.alpha / negative_samples_i.shape[1] *
