...
from cvpods.layers.metrics import accuracy
...
class BYOL(nn.Module):
    def __init__(self, cfg):
        ...
        self.predictor = nn.Sequential(
            nn.Linear(
                self.online_network.out_dim,
                self.online_network.proj_dim
            ),
    ...
    def forward(self, batched_inputs):
        ...
        x_linear = torch.stack([bi["image"] for bi in linear_inputs]).to(self.device)
        logits = self.linear_eval(torch.flatten(
            self.avgpool(self.online_network(x_linear)["res5"].detach()), 1))
